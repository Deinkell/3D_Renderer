...
void Render::MakePolygonNDCData(Vertex* _Polygon)
{
	for (int i = 0; i < 3; i++)
	{
		...
		_Polygon[i].Pos.W = _Polygon[i].Pos.W / _Polygon[i].Pos.W;
	}
}
		...
		{		
			...
			switch (Render_Type)
			{
				...
				case RenderType::FlatShading:
				{
					...
					{
						ThreadPool_Component->EnqueueJob([this]( Vertex tmp1, Vertex tmp2, Vertex tmp3)
							{ RasterizePolygon_Flat(tmp1, tmp2, tmp3, Color32(255,100,0)); }, tmp[0], tmp[1], tmp[2]);
					}//멀티스레드 렌더링
					else
						RasterizePolygon_Flat(tmp[0], tmp[1], tmp[2], Color32(255,100,0));
					//싱글스레드 렌더링
					break;
				}
